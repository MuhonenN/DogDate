...
class Role(db.Model):
    ...
    def insert_roles():
        roles = {
            'User': [Permission.SWIPE, Permission.FOLLOW, Permission.COMMENT, Permission.WRITE],
            'Moderator': [Permission.SWIPE, Permission.FOLLOW, Permission.COMMENT, Permission.WRITE, Permission.MODERATE],
            'Administrator': [Permission.SWIPE, Permission.FOLLOW, Permission.COMMENT, Permission.WRITE, Permission.ADMIN],
        }
        default_role = 'User'
        for r in roles:
...
class Post(db.Model):
    ...
    def to_json(self):
        json_post = {'url': url_for('api.get_post', id=self.id), 'body': self.body, 'body_html': self.body_html, 'timestamp': self.timestamp, 'author_url': url_for(
            'api.get_user', id=self.author_id), 'comments_url': url_for('api.get_post_comments', id=self.id), 'comment_count': self.comments.count()}
        return json_post
